...
// ImageTypeTuple represents one distro+arch+image_type combination
type ImageTypeTuple struct {
...
var imageTypesCache []ImageTypeTuple
...
func LoadImageTypesCache() error {
	...
	imageTypesCache = make([]ImageTypeTuple, len(rawEntries))
	for i, entry := range rawEntries {
		imageTypesCache[i] = ImageTypeTuple{
